...
$is_wizard = isset( $locals['is_wizard'] ) ? $locals['is_wizard'] : false;
...
?>
<form method="post" action="" id="ahrefs_seo_google_connected" class="ahrefs-seo-wizard ahrefs-analytics 
<?php
echo esc_attr( implode( ' ', $classes ) ); ?>">
	...
	<div class="card-item">
		...
		<div class="disconnect-wrap">
			...
			<?php
			esc_html_e( 'Your Google Analytics & Search Console accounts are connected', 'ahrefs-seo' );
			?>
			</div>
			<div class="your-account your-account-detecting">
			<?php
			esc_html_e( 'Autoselecting the best Google Analytics & Search Console profiles...', 'ahrefs-seo' );
			?>
			</div>
			<div class="your-account no-account-detected">
			<?php
			esc_html_e( 'Your Google account is connected but no suitable profiles were detected', 'ahrefs-seo' );
			?>
			</div>
			<?php
		...
		<?php
		$view->show_part( 'options/include/google-advanced', $locals );
		?>
	</div>
	<?php
