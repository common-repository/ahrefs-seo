...
<div class="input-wrap block-content">
	<a href="#" class="show-my-google-config show-collapsed block-subtitle"><?php // phpcs:ignore Squiz.PHP.EmbeddedPhp.ContentBeforeOpen,Squiz.PHP.EmbeddedPhp.ContentAfterOpen -- no additional space.
	if ( $is_wizard ) {
	...
	>
				<?php
				echo esc_textarea( $tokens_string );
				?></textarea><?php // phpcs:ignore Squiz.PHP.EmbeddedPhp.ContentAfterEnd,Squiz.PHP.EmbeddedPhp.ContentBeforeOpen,Squiz.PHP.EmbeddedPhp.ContentAfterOpen
if ( $google_connected ) {
	?>
					<a href="#" class="button button-large with-icon copy-clipboard-button" id="google_config_button">
					<?php
					esc_html_e( 'Copy to clipboard', 'ahrefs-seo' );
					?>
		</a>
			<?php
}
