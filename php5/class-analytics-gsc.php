...
trait Analytics_Gsc {
	 ...
	 * @return int|null
	 */
	protected function check_gsc_using_bulk_results( $gsc_site ) {
		$result = [];
		...
		foreach ( $urls as $url ) {
			$result[] = ! empty( wp_parse_url( $url, PHP_URL_PATH ) ) ? wp_parse_url( $url, PHP_URL_PATH ) : '';
		}
	 ...
	 */
	protected function check_gsc_using_bulk_results_strings( $gsc_site, $with_clicks_only = true ) {
		...
		foreach ( $response_total->getRows() as $row ) {
			if ( $row instanceof ApiDataRow ) {
				if ( ! $with_clicks_only || $row->getClicks() > 0 ) {
					// use only pages with traffic > 0.
					// key[0] is a page url.
					$result[] = isset( $row->getKeys()[0] ) ? $row->getKeys()[0] : '';
				}
			}
		}
		return $result;
	 ...
	 * @return bool
	 */
	private function is_enabled_and_set() {
		// is GSC enabled?
	 ...
	 * @since 0.10.1
	 */
	private function answer_has_error( $answer, $url, array &$results, $key ) {
	 ...
	 */
	public function get_clicks_and_impressions_by_urls( array $data_url_and_country_list, $start_date = null, $end_date = null, $limit = null, $without_totals = false, array $data_keyword_list = [] ) {
		...
		try {
			...
			foreach ( $data_url_and_country_list as $key => $data_keyword ) {
				...
				if ( ! $without_totals ) {
					$this->prepare_gsc_query( "{$key}-total", $batch, $service_searchconsole, $this->get_data_tokens()->get_gsc_site(), array_merge( $parameters, [ 'dimensions' => [ 'page' ] ] ) );
				}
				// Top 10 clicks, positions, impressions.
				$this->prepare_gsc_query( "{$key}-q", $batch, $service_searchconsole, $this->get_data_tokens()->get_gsc_site(), array_merge( $parameters, [ 'dimensions' => [ 'query', 'page' ] ] ) );
			}
			$e = null;
			try {
				// execute requests.
		...
		if ( count( $data_keyword_list ) ) {
			...
			foreach ( $data_keyword_list_ as $item ) {
				$data_keyword_list[ "{$item->get_url()}|" . strtolower( ! empty( $item->get_keyword() ) ? $item->get_keyword() : '' ) . '|' . $item->get_country_code() ] = $item;
			}
	 ...
	 * @return string
	 */
	protected function set_scheme_for_url( $url ) {
	 ...
	 * @since 0.9.4
	 */
	private function url_for_gsc( $url ) {
	 ...
	 * @since 0.7.3
	 */
	protected function prepare_gsc_query( $key, Google_Http_Batch &$batch, Google_Service_SearchConsole $service_searchconsole, $gsc_site, array $parameters ) {
		try {
			$post_body = new SearchAnalyticsQueryRequest( $parameters );
	 ...
	 * @since 0.7.3
	 */
	protected function parse_gsc_response( SearchAnalyticsQueryResponse $response = null ) {
		$result = [];
		if ( ! empty( $response ) ) {
	 ...
	 */
	public function get_position_fast( array $list_url_keyword ) {
		...
		try {
			...
			foreach ( $list_url_keyword as $key => $row ) {
				$url             = $this->url_for_gsc( ! empty( $row->get_url() ) ? $row->get_url() : '' );
				...
				$this->prepare_gsc_query( "{$key}-f", $batch, $service_searchconsole, $this->get_data_tokens()->get_gsc_site(), $parameters );
			}
			try {
				// execute requests.
				$this->maybe_do_a_pause( 'gsc' );
		...
		foreach ( $list_url_keyword as $key => $row ) {
			...
			$url    = ! empty( $row->get_url() ) ? $row->get_url() : '';
	 ...
	 * @since 0.9.4
	 */
	public function get_top_gsc_results() {
	 ...
	 */
	public function gsc_check_domain() {
		$site_domain = $this->get_clean_domain();
		// single or multiple domains.
		$analytics_domains = $this->is_ua_set() ? array_map( [ $this, 'get_clean_domain' ], explode( '|', $this->get_data_tokens()->get_ua_url() ) ) : [];
