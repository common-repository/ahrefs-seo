...
class Settings_Scope {
	 ...
	 */
	public function load_all_options_from_request() {
		$result = false;
		$hashes = [];
		foreach ( $this->options as $option ) {
	 ...
	 */
	private function get_tax_types_list() {
		static $result = null;
		if ( is_null( $result ) ) {
			$result = [];
			foreach ( [
	 ...
	 * @return bool Success.
	 */
	public function set_posts_categories_checked( $post_type, $taxonomy, array $values ) {
		foreach ( $this->options as $option ) {
	 ...
	 */
	public function get_posts_categories_checked( $post_type, $taxonomy ) {
		foreach ( $this->options as $option ) {
			if ( $option instanceof Option_Post && $post_type === $option->get_post_type() && ! $option->is_id_mode_used() ) {
				return ! empty( $option->load_options()->get_tax_values( $taxonomy ) ) ? $option->load_options()->get_tax_values( $taxonomy ) : [];
			}
		}
		return [];
	}
	 ...
	 * @return int[]|string[] Checked terms ID list.
	 */
	public function get_posts_id_checked( $post_type ) {
		foreach ( $this->options as $option ) {
			if ( $option instanceof Option_Post && $post_type === $option->get_post_type() && $option->is_id_mode_used() ) {
	 ...
	 * @return bool
	 */
	public static function is_enabled_for_taxonomy( $taxonomy ) {
	 ...
	 * @return void
	 */
	public function pages_add_to_checked( array $post_taxes ) {
		$list = array_filter(
			$post_taxes,
	 ...
	 * @return void
	 */
	public function pages_remove_from_checked( array $post_taxes ) {
		$list = array_filter(
			$post_taxes,
			function ( Post_Tax $item ) {
				return $item->is_post( 'page' );
