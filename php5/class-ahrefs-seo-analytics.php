...
class Ahrefs_Seo_Analytics extends Ahrefs_Seo_Abstract_Api {
	...
	const NONCE_INTERNAL_REDIRECT   = 'ahrefs-seo-google-nonce-1';
	 ...
	 */
	const REQUEST_SIZE_GA = 2;
	/**
	 * How many items load per request if GA4 profile used.
	 */
	const REQUEST_SIZE_GA4     = 20;
	const TRAFFIC_TYPE_TOTAL   = 'total';
	const TRAFFIC_TYPE_ORGANIC = 'Organic Search';
	...
	public function __construct() {
		$this->api_user           = substr( 'w' . implode( '-', [ get_current_user_id(), get_current_blog_id(), ! empty( wp_parse_url( get_home_url(), PHP_URL_HOST ) ) ? wp_parse_url( get_home_url(), PHP_URL_HOST ) : '' ] ), 0, 40 );
	 ...
	 * @return bool
	 */
	public function check_token( $code, $new_client_id = null ) {
	...
	private function handle_exception( Exception $e, $set_service_error = false, $is_gsc = false, $save_message = true ) {
		...
		if ( $e instanceof Google_Service_Exception ) {
			...
			if ( is_array( $error ) && isset( $error['error'] ) && in_array( $error['error'], [ 'invalid_grant', 'unauthorized_client' ], true ) ) {
				...
				if ( is_array( $errors ) && 0 < count( $errors ) && isset( $errors[0]['reason'] ) ) {
					...
					if ( 'forbidden' === $reason ) {
						if ( $is_gsc && $this->get_data_tokens()->get_gsc_site() ) { // if was not disconnected before.
							$site = preg_match( "/site '([^']+)'/", isset( $errors[0]['message'] ) ? $errors[0]['message'] : $e->getMessage(), $m ) ? $m[1] : $this->get_data_tokens()->get_gsc_site();
							// get site from message.
							$message = ! empty( $site ) ? sprintf( __( 'Your Google account has been disconnected because you don’t have the required permission for %s site.', 'ahrefs-seo' ), $site ) : __( 'Your Google account has been disconnected because you don’t have the required permission for this site.', 'ahrefs-seo' );
							$this->set_gsc_disconnect_reason( $message );
							$this->set_message( $message, $e ); // this will submit error.
							$no_report = true;
						} else {
							$ga      = $this->get_data_tokens()->get_ua_id();
							$message = ! empty( $ga ) ? sprintf( __( 'Your Google account has been disconnected because you don’t have the sufficient permissions for %s profile.', 'ahrefs-seo' ), $ga ) : __( 'Your Google account has been disconnected because you don’t have the sufficient permissions for this profile.', 'ahrefs-seo' );
				...
				if ( is_array( $errors ) && 0 < count( $errors ) && isset( $errors[0]['reason'] ) ) {
					$err = $errors[0];
					if ( 'invalidParameter' === $err['reason'] && 'siteUrl' === ( isset( $err['location'] ) ? $err['location'] : '' ) ) {
						/* Translators: %s: original error message */
						$message = sprintf( __( 'Your Google account has been disconnected because of error: %s', 'ahrefs-seo' ), isset( $err['message'] ) ? $err['message'] : $err['reason'] );
						$this->set_gsc_disconnect_reason( $message );
						$this->set_message( $message, $e ); // this will submit error.
						$no_report = true;
					}
	 ...
	 * @return void
	 */
	public function set_gsc_disconnect_reason( $string = null, $reset_gsc_account = true ) {
	 ...
	 * @since 0.8.4
	 */
	public function reset_pause( $reset_ga, $reset_gsc ) {
		if ( $reset_ga ) {
	 ...
	 * @since 0.9.11
	 */
	protected function notice_account_is_disconnected() {
	 ...
	 * @since 0.9.11
	 */
	public static function save_message( $message = null, $type = null ) {
