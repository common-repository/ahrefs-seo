...
class Hash
{
    ...
    {
        if (!\defined('ahrefs\\AhrefsSeo_Vendor\\CRYPT_HASH_MODE')) {
            switch (\true) {
                case \extension_loaded('hash'):
                    \define('ahrefs\\AhrefsSeo_Vendor\\CRYPT_HASH_MODE', self::MODE_HASH);
                    break;
                case \extension_loaded('mhash'):
                    \define('ahrefs\\AhrefsSeo_Vendor\\CRYPT_HASH_MODE', self::MODE_MHASH);
                    break;
                default:
                    \define('ahrefs\\AhrefsSeo_Vendor\\CRYPT_HASH_MODE', self::MODE_INTERNAL);
            }
        }
        $this->setHash($hash);
    }
    ...
    function _sha512($m)
    {
        ...
        if (!isset($k)) {
            ...
            for ($i = 0; $i < 8; $i++) {
                $init384[$i] = new \ahrefs\AhrefsSeo_Vendor\phpseclib\Math\BigInteger($init384[$i], 16);
                $init384[$i]->setPrecision(64);
                $init512[$i] = new \ahrefs\AhrefsSeo_Vendor\phpseclib\Math\BigInteger($init512[$i], 16);
            ...
            for ($i = 0; $i < 80; $i++) {
                $k[$i] = new \ahrefs\AhrefsSeo_Vendor\phpseclib\Math\BigInteger($k[$i], 16);
            }
        }
        ...
        foreach ($chunks as $chunk) {
            $w = array();
            for ($i = 0; $i < 16; $i++) {
                $temp = new \ahrefs\AhrefsSeo_Vendor\phpseclib\Math\BigInteger($this->_string_shift($chunk, 8), 256);
