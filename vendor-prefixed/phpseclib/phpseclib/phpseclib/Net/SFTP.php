 ...
 * @access  public
 */
class SFTP extends \ahrefs\AhrefsSeo_Vendor\phpseclib\Net\SSH2
{
    /**
    ...
    function __construct($host, $port = 22, $timeout = 10)
    {
        ...
        if (!\defined('ahrefs\\AhrefsSeo_Vendor\\NET_SFTP_QUEUE_SIZE')) {
            \define('ahrefs\\AhrefsSeo_Vendor\\NET_SFTP_QUEUE_SIZE', 32);
        }
        if (!\defined('ahrefs\\AhrefsSeo_Vendor\\NET_SFTP_UPLOAD_QUEUE_SIZE')) {
            \define('ahrefs\\AhrefsSeo_Vendor\\NET_SFTP_UPLOAD_QUEUE_SIZE', 1024);
        }
    }
    /**
    ...
    function _precheck()
    {
        if (!($this->bitmap & \ahrefs\AhrefsSeo_Vendor\phpseclib\Net\SSH2::MASK_LOGIN)) {
            return \false;
        }
        if ($this->pwd === \false) {
    ...
    function put($remote_file, $data, $mode = self::SOURCE_STRING, $start = -1, $local_start = -1, $progressCallback = null)
    {
        ...
        if ($mode & \ahrefs\AhrefsSeo_Vendor\phpseclib\Net\SFTP::SOURCE_LOCAL_FILE) {
    ...
    function _send_sftp_packet($type, $data, $request_id = 1)
    {
        ...
        if (\defined('ahrefs\\AhrefsSeo_Vendor\\NET_SFTP_LOGGING')) {
